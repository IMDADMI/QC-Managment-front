<template>
    <main-header />
    <BoxPr title="Stockage" lien="/shop/Composants/Stockage" lienComp=true> <img src="@/assets/images/Stockage.png" class="image1"></BoxPr><br>
            ...
            if (this.category.length > 0) {
                filters.push(product => {
                    return this.category.some(cat => {
                        if (cat.Nom === "Stockage SSD") {
                            
                                return product.productType === "Stockage" && /SSD/i.test(product.productName);
                            
                        } else if (cat.Nom === "Stockage HDD") {
                                return product.productType === "Stockage" && /HDD/i.test(product.productName);
                        }
                        return false;
                    });
                });
    ...
    methods: {
        handleResize() {
            ...
            axios.get('http://164.92.134.202:8099/api/product/allProduct').then((res) => {
                this.products = res.data.map((product) => {
                    if(product.productType=="Stockage"){
        ...
        getAllBrands() {
            ...
            this.products.forEach((Pro) => {
                ...
                if (/HDD/i.test(Pro.productName))
                    this.Categories.forEach((Cat) => {
                        if (Cat.Nom == "Stockage HDD")
                            Cat.quantite++;
