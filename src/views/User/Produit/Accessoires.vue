<template>
    <main-header />
    <BoxPr title="Accessoires" lien="/shop/Accessoires"></BoxPr><br>
    <h3 id="Types">les Types des Accessoires</h3>
    <div class="position">
...
export default {
    ...
    data() {
        ...
        selectCategorie() {
                ...
                filters.push(product => {
                    return this.category.some(cat => {
                        if (cat.Nom === "PC intel") {
                            ...
                            return product.configuartion.some(comp => {
                                return comp.component.compType === "Processeur" && /amd/i.test(comp.component.compName);
                            });
                        }
                        return false;
                    });
            ...
            if (this.CarteSelectionner !== 'Cartes Graphiques') {
                filters.push(product => {
                    return product.configuartion.some(comp => {
                        return comp.component.compType === "CarteGraphique" && comp.component.compName === this.CarteSelectionner;
                    });
                });
            }
            ...
            if (this.RAMSelectionner !== 'RAM') {
                filters.push(product => {
                    return product.configuartion.some(comp => {
                        return comp.component.compType === "RAM" && comp.component.compName === this.RAMSelectionner;
                    });
                });
            }
            ...
            if (this.SSDSelectionner !== 'Stockage SSD') {
                filters.push(product => {
                    return product.configuartion.some(comp => {
                        return comp.component.compType === "DisqueSSD" && comp.component.compName === this.SSDSelectionner;
                    });
                });
            }
            ...
            if (this.HDDSelectionner !== 'Stockage HDD') {
                filters.push(product => {
                    return product.configuartion.some(comp => {
                        return comp.component.compType === "DisqueHDD" && comp.component.compName === this.HDDSelectionner;
                    });
                });
            }
    ...
    methods: {
        handleResize() {
            ...
            axios.get('http://164.92.134.202:8099/api/product/allProduct').then((res) => {
                this.products = res.data.map((product) => {
                  if( this.SeulementCompo(product.productType)){
        ...
        },SeulementCompo(ProduitType){
            if(/Casque/i.test(ProduitType) || /Clavier/i.test(ProduitType) ||/Usb/i.test(ProduitType)||/Hautparleur/i.test(ProduitType)||/Microphone/i.test(ProduitType)||/Souris/i.test(ProduitType)||/Webcam/i.test(ProduitType))
            return true;
            else 
            return false;
